...
(defn in-checko
  [color]
   (cl/fresh [to opp-color opp-piece from] 
...
(defn mates-in-n-moveso
  [color N MFlag]
   (cl/conda
     [(cl/fresh [piece from to opp-color fromS toS pieceS move-num ply move-id]
                (legal-moveo color piece from to)
                (oppcol color, opp-color) 
                (make-moveo color, piece, from, to)
                ...
                (cl/project [N pieceS fromS toS move-num ply move-id]
                         ;;(log-move "White probe:" pieceS fromS toS move-num ply move-id)
                         (cle/ignoreo (mated-in-n-moveso opp-color, N, MFlag))
                         (unmake-moveo color, piece, from, to)
                         (cl/== MFlag 1)
                         (log-move "Mating move:" pieceS fromS toS move-num ply move-id))
                ;(ignoreo (cl/is PV PV #(conj % [ N color piece from to]  )))
                )
      (cl/log "Mating move found!")
      ]
     [cl/s#
  ...
  (cl/project [color]
              ...
              (cl/condu
                [(cl/all (cl/== N 1) 
                 ...
                 [(cl/fresh [piece from to opp-color NewN fromS toS pieceS move-num ply move-id]
                            (legal-moveo color piece from to)
                            (make-moveo color, piece, from, to)
                            (get-move-ido move-id)
                            ; (log-move "Make-black-move " piece from to nil nil move-id)
                            (piece-to-stringo color piece pieceS)
                            (field-to-stringo from fromS)
                            (field-to-stringo to toS)
                            (get-move-countero move-num)
                            (get-ply-countero ply)
                            (cl/project [N pieceS fromS toS move-num ply move-id]
                                        ;;(log-move "........................... Black refute:"
                                        ;;          pieceS fromS toS move-num ply move-id)
                                        (oppcol color, opp-color) 
                                        (cl/is NewN N dec)
                                        (cle/ignoreo (mates-in-n-moveso opp-color, NewN, MFlag))
                                        (unmake-moveo color, piece, from, to)
                                        (cl/== MFlag 0)
                                        ))
                  ]
...
(defn search-mateo 
  ...
  (cl/fresh [is-mated?]
            ...
            (set-move-countero 0)
            (set-ply-countero 0)
            (cle/ignoreo(mates-in-n-moveso color, N, is-mated?))
            (cl/== is-mated? 1)))

(defn mate-in
  [n]
  (time (doall (cl/run-db* @emil-db [q nodes]
                    (search-mateo :w n)
                    (get-node-countero nodes)
;                    (cl/project [nodes]
;                                (cl/log (str "Nodes visited: " nodes)))
                    (cl/== q "Found mate, nodes visited:")))))
         ...
         ;        
         ;% Zug ausgeben:
         ;writeZug(color, Figur, Von, Nach) :-
         ;        square-occupiedo(Nach), !,       % Schlagzug
