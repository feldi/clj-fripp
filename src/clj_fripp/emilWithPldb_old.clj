...
(defn assert-emil 
  ([fact] (assert-emil nil fact))
  ([a fact]
    (reset! emil-db (apply pldb/db-fact @emil-db fact))
    (when a
      (vary-meta a assoc-in [:db] (list @emil-db))
      ) )
    )
    
(defn retract-emil 
  ([fact] (retract-emil nil fact))
  ([a fact]
...
(defn retract-all-pos-alt
  "delete all 'pos' facts, i.e. clear the board."
  [] 
  (apply pldb/db-retractions @emil-db
   (cl/run-db* @emil-db [q]
   	  (cl/fresh [x y z]
   	    (pos x y z)
        (cl/== q (list 'pos x y z) )))))
...
(defn makeZug
  ...
  (cl/project [Farbe, Figur, Von, Nach] 
              (cl/fresh [Gegfarbe, Gegfigur]
                        (cl/conda
                          [(pos Gegfarbe Gegfigur Nach)
                           ;;(cl/trace-lvars "x:" Gegfarbe Gegfigur Nach) 
...
;% Benutzerzug einlesen und ausführen
;% Zugrep = 'd2d4', 'g8f6' etc.
;makeUserZug(Zugrep) :-
;        readZug(Zugrep, Von, Nach),
;        pos(Farbe, Figur, Von),
;        makeZug(Farbe,Figur,Von,Nach).
;
;
...
;      [(imSchach Farbe)
;       (cl/log "1. pruef")
;       (cl/log "vor unmake legalZug-2")
;       ;(cl/trace-lvars "unmake 2" Farbe, Figur, Von, Nach)
;       (unmakeZug Farbe, Figur, Von, Nach)
;       (cl/log "did unmake legalZug-2")
;      ]
;      [(cl/all 
;         (cl/log "2. pruef")
;         (unmakeZug Farbe, Figur, Von, Nach)
;         (cl/log "did unmake legalZug-3")
;         )
;       ])) 
;  )
...
(defn patt 
  [Farbe] 
   (cl/fresh [Figur Von Nach] 
             (cl/log "Pattprüfung")
             (cl/nafc imSchach Farbe)
             (cl/nafc legalZug Farbe Figur Von Nach))
)
...
;play(Farbe) :- pseudoZug(Farbe, Figur, Von, Nach),
;                writeZug(Farbe, Figur, Von, Nach),
;                makeZug(Farbe, Figur, Von, Nach), 
;                listing(pos),
;                oppcol(Farbe, GegFarbe),
;                pseudoZug(GegFarbe, GegFigur, GegVon, GegNach),
;                writeZug(GegFarbe, GegFigur, GegVon, GegNach),
;                unmakeZug(Farbe, Figur, Von, Nach),
;                fail.
;*/
  ...
  [Farbe N MFlag]
   (cl/conda
     [(cl/fresh [Figur Von Nach GegFarbe]
                ...
                (oppcol Farbe, GegFarbe) 
                (makeZug Farbe, Figur, Von, Nach)
                (cl/trace-lvars "prüfzug:" N Farbe Figur Von Nach GegFarbe) 
                ;(executeo (show-pos))
                (ignoreo (wirdMattInGenauN GegFarbe, N, MFlag))
                ; (executeo (show-pos))
                (cl/trace-lvars "nehme prüfzug zurück, ergebnis: " N Farbe Figur Von Nach MFlag) 
                (unmakeZug Farbe, Figur, Von, Nach)
                (cla/= MFlag 1)
                cl/s#
                )]
...
(defn wirdMattInGenauN
  ...
  (cl/conda
    ...
    [(cl/fresh [Figur Von Nach GegFarbe NewN]
              (cl/log " rekursiver abstieg")
              (legalZug Farbe Figur Von Nach)
              (makeZug Farbe, Figur, Von, Nach)
              (cl/trace-lvars "prüfe entgegnung:" N Farbe Figur Von Nach) 
              (oppcol Farbe, GegFarbe) 
              (cl/is NewN N #(dec %))
              (ignoreo (setztMattInGenauN GegFarbe, NewN, MFlag))
              (unmakeZug Farbe, Figur, Von, Nach)
              (cl/trace-lvars "nehme zurück prüfe entgegnung. ergebnis:" N Farbe Figur Von Nach NewN MFlag) 
              (cla/= MFlag 0)
              cl/s#
              )]
    [cl/s#
